...
def sneaktorious_run(db):
    ...
    db_docs = db.fetch_docs({"provider": "sneaktorious"}, {"_id": 0})
    ...
    for prod in requested_products:
        ...
        # If the product already exists and send ping is False, skip updating
        ...
        # Initialize custom_fields field
        custom_fields = {
            "Regions": [],
            "Number Of Raffles": prod["rafflesCount"]
        }

        # Fetch the regions where the product is going to be released
        for region in prod["region"]:
            custom_fields["Regions"].append(regions[region])

        # Set the custom_values field in prod_data
